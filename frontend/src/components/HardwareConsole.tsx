...
export function HardwareConsole({ logs, fullScreen = false }: Props) {
  ...
  return (
    <div className={`bg-[#0A0A0A] border border-slate-800 flex flex-col overflow-hidden w-full backdrop-blur-sm ${fullScreen ? 'h-full border-none' : 'max-h-48'}`}>
       ...
       <div className={`p-3 font-mono text-[9px] md:text-[11px] leading-relaxed flex flex-col gap-1 overflow-y-auto w-full break-all ${fullScreen ? 'flex-1' : 'h-28 md:h-32'}`}>
         {logs.length === 0 ? (

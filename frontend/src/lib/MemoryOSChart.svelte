<script lang="ts">
  ...
  function getCtxSwitchQuery() {
    return `hqud_os_context_switches_ps{host="${get(hwConfig).node_name}"}`;
  }
  ...
  async function fetchRange(query: string): Promise<[number, string][] | null> {
    ...
    const res = await fetch(
      `${VM_URL}?query=${encodeURIComponent(query)}&start=${start}&end=${now}&step=5`,
    );

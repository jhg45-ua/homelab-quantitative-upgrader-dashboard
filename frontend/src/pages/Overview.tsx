...
<p class="meta">Auto-generated by HQUD Foundry v2.5.0</p>
...
<table>
  ...
  <tr><td>Active Power</td><td>${metrics.powerW.toFixed(1)}</td><td>W</td></tr>
  <tr><td>CPU Efficiency</td><td>${(metrics.ipsPerW / 1e6).toFixed(2)}</td><td>M IPS/W</td></tr>
  <tr><td>CPI</td><td>${metrics.cpi.toFixed(3)}</td><td>cycles/instr</td></tr>
  <tr><td>AMAT</td><td>${metrics.amat.toFixed(2)}</td><td>cycles</td></tr>
  <tr><td>NUMA Miss Rate</td><td>${metrics.numaMiss.toFixed(2)}</td><td>%</td></tr>
  <tr><td>Cache Miss Rate</td><td>${metrics.cacheMiss.toFixed(2)}</td><td>%</td></tr>
  <tr><td>TCP Retransmits</td><td>${metrics.tcpRetrans.toFixed(1)}</td><td>/s</td></tr>
</table>

<h3>Top-Down Microarchitecture Analysis</h3>
<table>
  <tr><th>Category</th><th>%</th></tr>
  <tr><td>Retiring (useful work)</td><td>${metrics.tmaRetiring.toFixed(1)}%</td></tr>
  <tr><td>Bad Speculation</td><td>${metrics.tmaBadSpec.toFixed(1)}%</td></tr>
  <tr><td>Front-End Bound</td><td>${metrics.tmaFrontEnd.toFixed(1)}%</td></tr>
  <tr><td>Back-End Bound</td><td>${metrics.tmaBackEnd.toFixed(1)}%</td></tr>
</table>

<h3>Storage Subsystem (Little's Law)</h3>
<table>
  <tr><th>Queue Depth (blk_mq)</th><td>${metrics.queueDepth > 0 ? metrics.queueDepth.toFixed(0) : '—'}</td></tr>
  <tr><th>IOPS (Real)</th><td>${metrics.iops > 0 ? metrics.iops.toLocaleString() : '—'}</td></tr>
  <tr><th>Context Switches</th><td>${metrics.ctxSwitches.toFixed(0)} /s</td></tr>
</table>

<p class="meta" style="margin-top:32px;">End of Report. Generated at ${new Date().toLocaleString()}</p>
</body></html>`;
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="p-4 md:p-8 flex-1 flex flex-col gap-6 w-full max-w-[1600px] mx-auto overflow-y-auto">
        ...
        {/* System Configuration Metadata Panel — dynamic from config.yaml */}
        <div>
